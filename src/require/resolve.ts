...
abstract class AbstractResolve implements Resolve {
  ...
  protected async invalidate0(id: string, emit = true): Promise<void> {
    ...
    if (emit) {
      await this.onInvalidate.emit(id);
    }
  }
  ...
  implements Resolve
{
  ...
  protected readonly transpiled = new WeakMap<
    Transpile,
    WeakSet<AbstractFileResolve.Cache.Identity>
  >();
...
export namespace AbstractFileResolve {
  export class Cache {
    public readonly onInvalidate = new EventEmitterLite<
      readonly [path: string, cache: Cache.Identity | null]
    >();
    ...
    public [Symbol.iterator](): IterableIterator<
      readonly [string, Cache.Identity]
    > {
...
export class CompositeResolve implements Resolve {
  ...
  public constructor(delegates: readonly Resolve[]) {
  ...
  public async invalidate(
    ...
    await Promise.all(this.delegates.map(async (de) => de.invalidate(...args)));
  }
...
export class InternalModulesResolve extends AbstractResolve implements Resolve {
...
export class WikilinkResolve extends AbstractFileResolve implements Resolve {
  protected override resolvePath(id: string, context: Context): string | null {
    const {
        context: {
          app: { metadataCache },
...
function parseMarkdownLink(link: string): {
  ...
  const link2 = link.startsWith("!") ? link.slice("!".length) : link,
    [display, read] = parseComponent(link2, codePoint("\\"), [
      codePoint("["),
      codePoint("]"),
    ]);
  if (read < 0) {
  ...
  const rest = link2.slice(read),
    [pathtext, read2] = parseComponent(rest, codePoint("\\"), [
      codePoint("("),
      codePoint(")"),
    ]);
  if (read2 !== rest.length) {
    return null;
  }

  const pathParts = pathtext.split(/ +/u, 2),
    [, title] = /^"(?<title>(?:\\"|[^"])*)"$/u.exec(pathParts[1] ?? '""') ?? [];
  if (title === void 0) {
    return null;
  }
  return {
...
function parseWikilink(
  ...
  if (str !== link || path === void 0 || display === void 0) {
    return null;
  }
  return { display: display || (str.includes("|") ? "" : path), path };
}

export class ExternalLinkResolve extends AbstractResolve implements Resolve {
  protected readonly redirects = new Map<string, string>();
  protected readonly identities = new Map<
    string,
    AsyncOrSync<ExternalLinkResolve.Identity> | Error | "await"
  >();
  ...
  public constructor(
    ...
    const {
      ...
      preload = async (hrefs: readonly string[]): Promise<void> => {
        if (!settings.value.enableExternalLinks) {
          return;
        }
  ...
  public override resolve(id: string, context: Context): Resolved | null {
    ...
    if (!this.context.settings.value.enableExternalLinks) {
      return null;
    }
  ...
  > {
    ...
    if (identity === void 0 || identity === "await") {
      ...
      this.identities.set(
        href,
        (identity = (async (): Promise<ExternalLinkResolve.Identity> => {
          ...
          try {
            compile = async (): Promise<void> => {
              ret.compiledSyncCode = await (
                await this.workerPool
              ).exec<typeof attachSourceMap>("attachSourceMap", [
                {
                  code: ret.code,
                  id: href,
            ...
            const { code, requires } = await (
              await this.workerPool
            ).exec<typeof parseAndRewriteRequire>("parseAndRewriteRequire", [
              {
                code: ret.code,
                href,
